...
CREATE TABLE personnel (
    pseudo_personnel VARCHAR(27) PRIMARY KEY DEFAULT 'Pers' || nextval('pers_seq'),
...
CREATE TABLE reservation (
    ...
    status_reservation VARCHAR(27) NOT NULL CHECK (status_reservation IN ('en_attente', 'confirmee', 'annulee')),
    pseudo_joueur VARCHAR(27) NOT NULL,
    FOREIGN KEY (pseudo_joueur) REFERENCES joueur(pseudo_joueur) ON DELETE CASCADE
);
...
CREATE TABLE machine (
    ...
    statut_machine VARCHAR(50) NOT NULL CHECK (statut_machine IN ('disponible', 'maintenance', 'horsservice')),
...
-- Ajouter les contraintes de clé étrangère après avoir créé les tables
ALTER TABLE identite
ADD CONSTRAINT fk_identite_adresse
FOREIGN KEY (id_adresse) REFERENCES adresse(id_adresse);

ALTER TABLE adresse
ADD CONSTRAINT fk_adresse_identite
